...
$data = array();
$sql = "select event_details.id, event_details.organizer_id, event_details.name, event_details.city, event_details.venue, event_details.start_date, event_details.start_time, event_details.end_date, event_details.end_time, event_details.category, event_details.description, event_details.image, event_details.total_people, ticket.ticket_required, ticket.cost_per_ticket, ticket.total_tickets, user.full_name from event_details inner join ticket on event_details.id = ticket.event_id inner join user on event_details.organizer_id = user.id inner join interested on event_details.id = interested.event_id where interested.user_id = $user_id ";
